...
```
    ...
    console.error("CHYBA DATABAZY:", error);
...
embed.setFooter({
    text: `CZ/SK/EN Global Ranking • ${theme.emoji} ${theme.name}`
});
    ...
    .addStringOption(option =>
        option
            ...
            .addChoices(
                ...
                { name: "🔥 Fire — oheň/plamene", value: "fire" },
                { name: "❄️ Snow — zasnežená krajina", value: "snow" },
                { name: "🧊 Ice — ľadovec", value: "ice" },
                ...
                { name: "💎 Diamond — diamanty", value: "diamond" },
                ...
                { name: "⛏️ Minecraft — Minecraft motív", value: "minecraft" },
...
new SlashCommandBuilder()
    ...
    .addUserOption(option =>
        option
            .setName("tester")
            .setDescription("Používateľ, ktorý sa stáva testerom")
            .setRequired(true)
    )
    ...
    .addStringOption(option =>
        option
            .setName("rank")
            .setDescription("POVINNÉ — rank testera")
            .setRequired(true)
    )
    ...
    .addIntegerOption(option =>
        option
            .setName("skusenosti")
            .setDescription("POVINNÉ — skúsenosti / XP")
            .setRequired(true)
            .setMinValue(0)
    )
...
console.log(`Registrujem ${commandData.length} slash príkazov...`);
...
console.log("SLASH PRIKAZY USPESNE ZAREGISTROVANE!");
...
```
client.user.setActivity("Global Ranking 🏆");

try {
    await registerCommands();
} catch (error) {
    console.error("REGISTRACIA PRIKAZOV ZLYHALA!");
    console.error(error);
}
```
...
try {
    ...
    if (interaction.commandName === "help") {
        ...
        const embed = createEmbed({
            userId: interaction.user.id,
            title: "🏆 CZ/SK/EN GLOBAL RANKING",
            description: "Všetky dostupné príkazy:",
            fields: [
                {
                    name: "🎨 /embed",
                    value: "Nastaví osobný embed motív."
                },
                {
                    name: "🏆 /addrank",
                    value: "Pridá výsledok rank testu."
                },
                {
                    name: "🧪 /settester",
                    value: "Povýši používateľa na testera."
                },
                {
                    name: "👤 /testerinfo",
                    value: "Zobrazí profil testera."
                },
                {
                    name: "👥 /testers",
                    value: "Zobrazí všetkých testerov."
                },
                {
                    name: "🚫 /removetester",
                    value: "Odoberie testerovi status."
                },
                {
                    name: "🏓 /ping",
                    value: "Skontroluje stav botu."
                }
            ]
        });
    ...
    if (interaction.commandName === "embed") {
        ...
        db.users[target.id].theme = themeKey;
        db.users[target.id].updatedBy = interaction.user.id;
        ...
        const embed = createEmbed({
            userId: target.id,
            title: `${theme.emoji} EMBED MOTÍV NASTAVENÝ`,
            description:
    ...
    if (
        ["addrank", "settester", "removetester"]
    ...
    if (interaction.commandName === "addrank") {
        ...
        const fields = [
            ...
            {
                name: "🔄 Kôl",
                value: rounds ? String(rounds) : "Neuvedené",
                inline: true
            }
        ];
        ...
        const embed = createEmbed({
            userId: player.id,
            title: `${result} • 🏆 RANK TEST`,
            description: `## ${player.username}`,
            color,
            fields,
            thumbnail: player.displayAvatarURL()
        });
    ...
    if (interaction.commandName === "settester") {
        ...
        const tests =
            interaction.options.getInteger("testy") ?? 0;

        const successful =
            interaction.options.getInteger("uspesne_testy") ?? 0;
        ...
        const successRate =
            tests > 0
                ? Math.round(successful / tests * 100)
                : 0;
        ...
        const embed = createEmbed({
            ...
            thumbnail: tester.displayAvatarURL()
        });
    ...
    if (interaction.commandName === "testerinfo") {
        ...
        const db = loadDB();
        const tester = db.testers[user.id];
        ...
        const successRate =
            tester.tests > 0
                ? Math.round(
                    tester.successful / tester.tests * 100
                )
                : 0;

        const embed = createEmbed({
            userId: user.id,
            title: `🧪 TESTER • ${user.username}`,
            fields: [
                {
                    name: "🎮 Gamemode",
                    value: tester.gamemode,
                    inline: true
                },
                {
                    name: "🏅 Rank",
    ...
    if (interaction.commandName === "testers") {
        ...
        const testers =
            Object.values(db.testers)
                .filter(tester => tester.active);
    ...
    if (interaction.commandName === "removetester") {
        ...
        db.testers[tester.id].active = false;
        db.testers[tester.id].removedBy = interaction.user.id;
        db.testers[tester.id].removedAt = new Date().toISOString();
        db.testers[tester.id].removeReason = reason;

        saveDB(db);

        const embed = createEmbed({
            userId: tester.id,
            title: "🚫 TESTER ODOBRATÝ",
            description:
    ...
    console.error("CHYBA PRI SPRACOVANÍ PRÍKAZU:");
    console.error(error);
...
client.login(TOKEN)
.then(() => {
console.log("Discord login úspešný.");
})
.catch(error => {
